...
class Pix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # specify the p2p_models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>
        if self.isTrain:
    ...
    def set_input(self, input):
        ...
        original_dose = input['A' if AtoB else 'B'].unsqueeze(dim=1)
        target_dose = input['B' if AtoB else 'A'].unsqueeze(dim=1)
        density = input['density'].unsqueeze(dim=1)
