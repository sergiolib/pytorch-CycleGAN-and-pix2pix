...
This script works for various p2p_models (with option '--model': e.g., pix2pix, cyclegan, colorization) and
different datasets (with option '--dataset_mode': e.g., aligned, unaligned, single, colorization).
You need to specify the dataset ('--dataroot'), experiment name ('--name'), and model ('--model').

It first creates model, dataset, and visualizer given the option.
It then does standard network training. During the training, it also visualize/save the images, print/save the loss plot, and save p2p_models.
...
See p2p_options/base_options.py and p2p_options/train_options.py for more training p2p_options.
...
if __name__ == '__main__':
    opt = TrainOptions().parse()   # get training p2p_options
    dataset = create_dataset(opt)  # create a dataset given opt.dataset_mode and other p2p_options
    dataset_size = len(dataset)    # get the number of images in the dataset.
    print('The number of training images = %d' % dataset_size)

    model = create_model(opt)      # create a model given opt.model and other p2p_options
